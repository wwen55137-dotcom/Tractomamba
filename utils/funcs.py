...
def clusters_to_tract_labels(lst, ordered_tract_cluster_mapping_dict, output_lst=True):
...
def write_tract_parcellation(args, pd_tractography, predicted_lst, label_names):
...
def str2num(rot_ang_lst_str):
    new_rot_ang_lst = list(map(float, rot_ang_lst_str.split(" ")))
