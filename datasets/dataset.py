...
class RealData_PatchData(data.Dataset):
    def __init__(self, feat, k, k_global, cal_equiv_dist=False, use_endpoints_dist=False, 
        ...
        if self.k_global==0:
            ...
            self.global_feat = self.global_feat.transpose(1,2,0).astype(np.float32)  # [n_point, n_feat, k_global] 
        ...
        if self.k==0:
            ...
            num_iter = max(1, num_fiber // self.rough_num_fiber_each_iter)
    ...
    def __getitem__(self, index):
        ...
        global_point_set = self.global_feat     # [n_point, n_feat, k_global]

        # ===== flip augmentation (direction reverse) =====
        if self.flip_aug and (np.random.rand() < self.flip_prob):
            point_set = point_set[::-1].copy()
            klocal_point_set = klocal_point_set[::-1].copy()
            global_point_set = global_point_set[::-1].copy()

        # numpy -> torch
        point_set = torch.from_numpy(point_set.astype(np.float32))
        klocal_point_set = torch.from_numpy(klocal_point_set.astype(np.float32))
        global_point_set = torch.from_numpy(global_point_set.astype(np.float32))

        return point_set, klocal_point_set, global_point_set
...
class unrelatedHCP_PatchData(data.Dataset):
    def __init__(self, root, out_path, logger, split='train', num_fiber_per_brain=10000,num_point_per_fiber=15, 
        ...
        # calculate local global features/representations
        self.org_feat, self.org_label, self.local_feat, self.global_feat, self.new_subidx = self._cal_info_feat()
        

    def __getitem__(self, index):
        point_set = self.org_feat[index]          # (n_point, n_feat)
        label = self.org_label[index]
        klocal_point_set = self.local_feat[index] # (n_point, n_feat, k)
        new_subidx = self.new_subidx[index]
        
        # 核心修改：获取对应被试的全局特征集
        sub_idx = int(new_subidx)
        global_point_set = self.global_feat[sub_idx] # (n_point, n_feat, k_global)

        # ===== flip augmentation (direction reverse) =====
        if self.flip_aug and (self.split in ['train', 'val']) and (np.random.rand() < self.flip_prob):
            point_set = point_set[::-1].copy()
            klocal_point_set = klocal_point_set[::-1].copy()
            global_point_set = global_point_set[::-1].copy()
        ...
        return point_set, label, klocal_point_set, global_point_set, new_subidx
    ...
    def _cal_brain_feat(self):
        ...
        for i_subject, unique_id in enumerate(unique_subject_ids):  # for each subject
            ...
            if self.aug_times > 0:
                ...
                cur_features_torch = torch.from_numpy(cur_features)  # numpy to tensor
    ...
    def _cal_info_feat(self):
        ...
        for cur_idx in range(num_subjects):
            ...
            if self.k_global>0:
                ...
                cur_global_feat = cur_feat[random_idx,...].transpose(2,1,0)  # [n_point, n_feat, k_global]
            # new sub idx
...
# def center_tractography(input_path, feat_RAS, out_path=None, logger=None, 
#                         tractography_name=None, save_data=False):
#     """
#     Rigid recenter (no shape distortion):
#     shift the entire subject tractography by ONE translation vector.

#     feat_RAS: [n_fiber, n_point, 3]
#     """
#     HCP_center = np.load(os.path.join(input_path, 'HCP_mass_center.npy')).astype(np.float32)

#     # atlas centroid: (3,)
#     if HCP_center.ndim == 2 and HCP_center.shape[1] == 3:
#         atlas_centroid = HCP_center.mean(axis=0)          # (3,)
#     elif HCP_center.ndim == 1 and HCP_center.shape[0] == 3:
#         atlas_centroid = HCP_center                       # (3,)
#     else:
#         raise ValueError(f"Unexpected HCP_center shape: {HCP_center.shape}")

#     feat_RAS = np.asarray(feat_RAS, dtype=np.float32)     # (F,P,3)

#     # subject centroid over all fibers & all points: (3,)
#     test_subject_centroid = feat_RAS.reshape(-1, 3).mean(axis=0)

#     displacement = atlas_centroid - test_subject_centroid  # (3,)
#     c_feat_RAS = feat_RAS + displacement[None, None, :]    # (F,P,3)
...
#         wma.io.write_polydata(
#             feat_RAS_pd, 
#             os.path.join(recenter_path, 'recentered_{}'.format(tractography_name))
#         )
